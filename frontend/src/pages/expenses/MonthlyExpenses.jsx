...
function MonthlyExpenses() {
  ...
  const monthStr = new Date().toISOString().slice(0, 7); // e.g. "2026-06"
  const [tdate, setTdate] = useState(monthStr);
  ...
  // Format month (e.g. June 2026)
  const formatHeaderMonth = (dateStr) => {
    const [year, month] = dateStr.split("-");
    const d = new Date(year, month - 1, 1);
    return d.toLocaleDateString("en-US", { month: "long", year: "numeric" });
  };

  // Format single transaction date for monthly listing (e.g. 15 Jun)
  ...
  // Get default date for adding new expense (e.g., today's date if it falls in the selected month, otherwise 1st of month)
  const getModalDefaultDate = () => {
    const today = new Date();
    const todayMonthStr = today.toISOString().slice(0, 7);
    if (todayMonthStr === tdate) {
  ...
  return (
    <div className="flex-1 flex flex-col p-4 relative overflow-hidden h-full">
        ...
        {loading ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center py-12 px-6">
            ...
            <p className="text-xs text-slate-400 dark:text-zinc-500 mt-1 max-w-[240px]">
              Tap on the plus icon below to record an expense for this month.
            </p>
          </div>
        ) : (
          <div className="space-y-3">
            <h3 className="text-xs font-bold text-slate-400 dark:text-zinc-500 uppercase tracking-wider px-1">
              Monthly Transactions ({expenses.length})
            </h3>
      ...
      {isModalOpen && (
        <AddExpenseModal
          key={editingExpense ? editingExpense._id : "new-monthly"}
